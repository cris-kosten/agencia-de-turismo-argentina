...
# ============================================================
# Persistencia de USUARIOS: archivo de texto CSV.
# (Las reservas y los paquetes se guardan en JSON, en sus propios módulos.)
...
def validar_dni(dni):
    # DNI: 7 u 8 digitos numericos (en Argentina puede tener 7).
    valido = True
    if not dni.isdigit():
        print("  El DNI solo puede tener numeros.")
        valido = False
    elif len(dni) < 7 or len(dni) > 8:
        print("  El DNI debe tener 7 u 8 digitos.")
        valido = False
    return valido
...
def validar_celular(celular):
    # Celular: al menos 10 digitos numericos (formato argentino).
    valido = True
    if not celular.isdigit():
        print("  El celular solo puede tener numeros.")
        valido = False
    elif len(celular) < 10:
        print("  El celular debe tener al menos 10 digitos.")
        valido = False
    return valido
...
def registrarse():
    ...
    valido = False
    while not valido:
        dni = input("  DNI (7 u 8 digitos): ").strip()
    ...
    while edad == -1:
        ...
        try:
            ...
            if not es_mayor_de_edad(fecha_nac):
                print("  Debes ser mayor de edad (18 o mas) para registrarte.")
                edad = -1
        except ValueError:
    ...
    valido = False
    while not valido:
        celular = input("  Celular (minimo 10 digitos): ").strip()
